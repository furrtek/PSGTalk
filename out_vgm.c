...
int out_vgm(unsigned char * out_buffer, unsigned int const * frequencies, unsigned int const * volumes) {
	...
	if (update_rate > 1) {
		...
		file_length = (framei + (framei * psg_channels * 7) + 1) + 64;		// Wrong ?
	} else
		file_length = (framei + (framei * psg_channels * 9) + 1) + 64;		// Wrong ?
	...
	data_idx = 64;	// Skip header for now
	...
	for (f = 0; f < framei - 1; f++) {
		...
		for (c = 0; c < psg_channels; c++) {
			...
			data_word = psg_internal / (frequencies[idx] * granu);
