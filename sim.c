...
int gensim(int ws, int framei, char channels, int freqs[], int vols[]) {
	...
	for (f=0; f<framei-1; f++) {
		...
		for (s=0; s<ws; s++) {
            sa = 128 + (square(0xFF * freqs[f*3] * s / ws) * vol_lut[vols[f*3]]);
            mix = sa;
            if (channels > 1) {
				sb = 128 + (square(0xFF * freqs[(f*3)+1] * s / ws) * vol_lut[vols[(f*3)+1]]);
            	mix += sb;
			}
			if (channels > 2) {
				sc = 128 + (square(0xFF * freqs[(f*3)+2] * s / ws) * vol_lut[vols[(f*3)+2]]);
				mix += sc;
			}
            b = (unsigned char)mix;
            wavout[s + (f * ws)] = b;
